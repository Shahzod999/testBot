...
const MainPage = () => {
  ...
  // Используем useRef для предотвращения бесконечных вызовов
  const hasCompanyIdSet = useRef(false);
  const hasLocationSet = useRef(false);
  ...
  useEffect(() => {
    if (hasCompanyIdSet.current) return;
    hasCompanyIdSet.current = true;
    ...
    if (initCompanyId) {
      ...
      if (storage) {
        storage.getItem("companyId", (error: any, value: string | null) => {
          if (error) {
            dispatch(setCompanyId(import.meta.env.VITE_COMPANYID));
          } else if (value) {
            const data = JSON.parse(value);
            if (data?.companyId) {
              dispatch(setCompanyId(data.companyId));
  ...
  useEffect(() => {
    if (hasLocationSet.current) return;
    hasLocationSet.current = true;
  ...
  useEffect(() => {
    try {
      const requiredVersion = "8.0";
      const currentVersion = tg?.version || "0";
