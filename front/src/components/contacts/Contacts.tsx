...
const Contacts = () => {
  ...
  const isBothNull = companyInfo.mobile_apps?.ios === null && companyInfo.mobile_apps?.android === null;

  console.log(isBothNull);

  const actions = [
    { text: "Скачать приложение", icon: <TbAppsFilled />, isDisabled: isBothNull },
    { text: "Telegram, WhatsApp", icon: <TbMessageChatbotFilled /> },
    { text: "+998 99 001 23 45", icon: <IoCall /> },
    { text: companyInfo.website.replace("https://", ""), icon: <IoEarth /> },
    { text: "Открыто до 22:00", icon: <GoClockFill /> },
    { text: companyInfo.address, icon: <IoNavigateCircle /> },
    { text: "Доступные вакансии (2)", icon: <FaAddressCard /> },
  ];
